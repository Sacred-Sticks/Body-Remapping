...
namespace InputManagement.Inputs
{
    ...
    public class InputManager : ScriptableObject
    {
        ...
        private void ReceiveInput(object sender, PlayerInputArguments e)
        {
            ...
            if (e.Variable.GetType() == typeof(QuaternionVariable)) 
            {
                var variable = e.Variable as QuaternionVariable;
                variable!.Value = e.Context.ReadValue<Quaternion>();
                return;
            }
            
            Debug.LogWarning($"Input Failed: {e.Binding}");
        }
    }
}